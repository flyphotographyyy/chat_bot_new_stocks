...
def compute_enhanced_indicators(df: pd.DataFrame, config: Dict) -> pd.DataFrame:
    ...
    for p in config.get("RSI",{}).get("periods",[14]):
        df[f'RSI{p}'] = rsi(close, p)

    fasts  = config.get("MACD",{}).get("fast",[12])
    slows  = config.get("MACD",{}).get("slow",[26])
    sigs   = config.get("MACD",{}).get("signal",[9])
    ...
    if "Bollinger" in config.get("indicators", []):
        bbp = config.get("Bollinger",{}).get("period",20)
        bbs = config.get("Bollinger",{}).get("std_dev",2)
...
def enhanced_signal_classification(ticker: str, df: pd.DataFrame, news_sent: Dict,
    ...
    # Fundamentals (fast_info only – optional)
...
def process_one(ticker: str, config: Dict):
    ...
    if df.empty:
        return None, None
    # add fundamentals into df (best-effort)
    fi = fetch_fast_info(ticker)
            ...
            with st.expander(f"{badge} {t} – {sig} (Score {score}, Confidence {conf}%)"):
                ...
                if show_charts:
                    try:
                        ...
                        if not df_chart.empty:
                            df_chart = compute_enhanced_indicators(df_chart, INDICATOR_CONFIGS)
                ...
                if enable_backtest:
                    try:
                        ...
                        if not df_bt.empty:
                            df_bt = compute_enhanced_indicators(df_bt, INDICATOR_CONFIGS)
